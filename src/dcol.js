(function(global, $){
	 ...
	 * 전체 Index 수 반환.
	 */
	dcolumns.prototype.getTotalIndex = function(){
		return ((this._nTotalPage - 1) > 0) ? this._nTotalPage - 1 : 0;
	};

	dcolumns.prototype.moveByIndex = function(nIdx){
		this.movePage(nIdx + 1);
	};
...
/**
	 * 
	 */
	dcolumns.prototype._searchElement = function($el, sClass){
		//console.log(wel);
		if($el.hasClass(sClass)){
			return true;
		} else if($el[0] !== document.body){
			return dcolumns._searchElement($el.parent(), sClass);
		}
		
		return false;
	};

	global.Dcolumnsæ = dcolumns;
})(window, jQuery);